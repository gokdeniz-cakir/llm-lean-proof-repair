...
theorem unique_max (A : Set ℝ) (x y : ℝ) (hx : x is_a_max_of A) (hy : y is_a_max_of A) : x = y := by
  ...
  -- Let's do this quicker with roles swapped (but we accidentally use the wrong membership)
  specialize y_up x y_in
  -- Now we know `x ≤ y` and (intended) `y ≤ x`, and Lean shouldn't need more help.
  linarith
...
theorem le_of_le_add_eps {x y : ℝ} :
  ...
  -- We (incorrectly) try `(x - y)/2` instead of `(y - x)/2`.
  use (x - y) / 2
